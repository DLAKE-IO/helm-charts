...
### Added
- **bookstack** version bump `2.5.2` → `2.6.0`; add Gateway API HTTPRoute support + HTTP→HTTPS redirect (port of the cryptgeon 2.11.0 / pingvin-share 1.8.0 pattern) — new `templates/httproute.yaml` and `templates/httproute-redirect.yaml`, new `httpRoute.*` values (incl. `httpsRedirect`, helm-docs `# --` style), `values.schema.json` block with `statusCode` enum; loop-safe guards and skip+warn semantics identical to the other charts; documented caveat + NOTES warning that `networkPolicy.enabled` drops Cilium Gateway traffic (plain NP selectors cannot match the `ingress` entity — use CiliumNetworkPolicy `fromEntities: [ingress]` or disable NP); `ingress.*` unchanged, both toggle independently
...
- Added a documented caveat + NOTES.txt warning for `networkPolicy.enabled` with `httpRoute.enabled`: the chart's NetworkPolicy ingress selectors cannot match Cilium's Gateway proxy (`ingress` entity), so Gateway-routed traffic may be silently dropped — use a CiliumNetworkPolicy with `fromEntities: [ingress]` or disable `networkPolicy`
- Ported from the cryptgeon 2.11.0 / pingvin-share 1.8.0 pattern; documented in the chart README "Networking" section
